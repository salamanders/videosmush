...
/**
 * A new, high-performance image averager that uses a parallel approach.
 */
class AveragingImage3
private constructor(
    val width: Int,
    val height: Int,
    private val sums: LongArray = LongArray(width * height * 3),
     ...
     */
    operator fun plusAssign(other: BufferedImage) = runBlocking {
        numAdded++
        require(numAdded * 255 < Long.MAX_VALUE) { "Possible overflow in DecodedImage after $numAdded adds." }
                ...
                coroutineScope {
                    for (y in 0 until height) {
                        launch(context) {
                            for (x in 0 until width) {
                                val i = y * width + x
                                val r = (sums[i * 3 + 0] / numAdded).toInt() and 0xFF
                                val g = (sums[i * 3 + 1] / numAdded).toInt() and 0xFF
                                val b = (sums[i * 3 + 2] / numAdded).toInt() and 0xFF
    ...
    companion object {
        ...
        internal fun blankOf(width: Int, height: Int): AveragingImage3 =
            AveragingImage3(width = width, height = height).also {
                it.numAdded = 0
            }
    }
}
